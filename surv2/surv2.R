...
## correlations between osa resid and resid
s_corrs = list()
a_corrs = list()
y_corrs = list()
...
simdata=expand.grid(age=age,survey=1:2,year=1:T) 

simdata$year.eff = year.eff[simdata$year]  
simdata$age.eff = age.eff[simdata$age]  

################  the q misspecification for survey=2  #############################
ind = simdata$year>20 & simdata$survey==2
tyear = simdata$year[ind]
simdata$age.eff[ind]=simdata$age.eff[ind]-2*(tyear-20)/max(tyear-20)
...
if(ordering == "say"){
    ind = order(simdata$survey,simdata$age,simdata$year)
    subdir2 = "./sort_survey_age_year"
}else if(ordering == "asy"){
    ind = order(simdata$age,simdata$survey,simdata$year)
    subdir2 = "./sort_age_survey_year"
}else if(ordering == "sya"){
    ind = order(simdata$survey,simdata$year,simdata$age)
    subdir2 = "./sort_survey_year_age"
}else if(ordering == "yas"){
    ind = order(simdata$year,simdata$age,simdata$survey)
    subdir2 = "./sort_year_age_survey"
...
map = list(q = factor(matrix(c('q11','q12',NA,'q14','q15','q21','q22',NA,'q24','q25'),2,A,byrow=T))) 
...
onam = paste0("./sims/outputs",nseed,"-",ordering,".rds")
